...
function showPasswordReset(){
  ...
  $('#saveNewPassword').onclick=async()=>{const p=$('#newPassword').value;if(p.length<10)return toast('Mindestens 10 Zeichen');const {error}=await sb.auth.updateUser({password:p});if(error)return toast(error.message);recoveryMode=false;sessionStorage.removeItem(SENSITIVE_AUTH_UNTIL_KEY);try{history.replaceState(null,'',cleanBaseUrl())}catch{}const {error:signOutError}=await sb.auth.signOut({scope:'global'});prodSession=null;serverLoaded=false;realtimeChannel?.unsubscribe();realtimeChannel=null;if(signOutError){productionError(signOutError,'Alle Sitzungen abmelden');return showAuthGate('login','Passwort wurde geändert. Bitte melde dich mit dem neuen Passwort an. Andere Geräte werden beim nächsten Sitzungsabgleich abgemeldet.','ok')}showAuthGate('login','Passwort geändert ✓ Aus Sicherheitsgründen wurden alle Geräte abgemeldet. Bitte melde dich mit dem neuen Passwort an.','ok')}
}
